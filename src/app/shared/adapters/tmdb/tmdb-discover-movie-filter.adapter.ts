...
export class TmdbDiscoverMovieAdapter implements FilterAdapter<DiscoverMovieFilters, Record<string, string>> {

    private readonly mapping: Record<keyof DiscoverMovieFilters, TmdbFilterMapping> = {
        withGenres: {
        ...
        releaseDateLte: { type: tmdbMappingTypes.DATE, externalKey: "release_date.lte", multi: false, allowedValues: [] },
    ...
    encode(filterSet: FilterSet<DiscoverMovieFilters>): AdapterResult<Record<string, string>> {
        const result: Record<string, string> = {};
        const errors: AdapterError[] = [];

        for (const key in filterSet) {
            const filter = filterSet[key as keyof DiscoverMovieFilters];
            if (!filter) continue;
            ...
            const mapped = this.mapping[key as keyof DiscoverMovieFilters];
